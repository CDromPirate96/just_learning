...
def get_random_account():
    """Getting data from random account"""
    return random.choice(data)

def format_data(account):
    """Formating account into printable format: name, description and country"""
    name = account["name"]
...
def check_answer(guess, a_followers, b_followers):
    """Checking followers against the user's guess and returning True if they got it right. Or False if they got it wrong."""
...
to B - Selena Gomez (174k). Instagram has more followers, so choice A is correct. However,
the subsequent comparison should be between Selena Gomez (the new A) and someone else.
